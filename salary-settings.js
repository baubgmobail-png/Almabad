...
function load(){try{const raw=JSON.parse(localStorage.getItem(STORAGE_KEY));if(!raw)return emptyState();return{...emptyState(),...raw,settings:{...DEFAULT_SETTINGS,...(raw.settings||{})},records:raw.records||{},shiftChanges:Array.isArray(raw.shiftChanges)?raw.shiftChanges:[],salaryIncreases:Array.isArray(raw.salaryIncreases)?raw.salaryIncreases:[]}}catch{return emptyState()}}
let state=load();
...
function renderChanges(){
  ...
  if(!state.shiftChanges.length){box.innerHTML='<div class="sf-note">ما في تغييرات إضافية. النظام الأساسي بالأعلى هو المستخدم.</div>';return}
  box.innerHTML=state.shiftChanges.map((x,i)=>`<div class="change-row" data-index="${i}">
  <label>ساري من<input type="date" data-change="effectiveFrom" value="${x.effectiveFrom||''}"></label>
